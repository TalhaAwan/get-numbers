...
describe('ExtractNumbers', () => {
  ...
  test('get numbers with commas', () => {
    ...
    expect(extractor.extractNumbers('10,000 and 15,000 respectively')).toEqual([
      '10,000',
      '15,000',
    ]);
    expect(
      extractor.extractNumbers('100,000,000,000 is a large amount, and so is 900,000,000,000,000')
    ).toEqual(['100,000,000,000', '900,000,000,000,000']);
  });
  ...
  test('get decimals with comma', () => {
    ...
    expect(
      extractor.extractNumbers('Your balance: $100,000.77, previous month: $90,899.89')
    ).toEqual(['100,000.77', '90,899.89']);
    expect(
      extractor.extractNumbers('Your balance: 100,000,000.77, previous month: 90,899,232.89')
    ).toEqual(['100,000,000.77', '90,899,232.89']);
  });

  test('get negative numbers', () => {
    const extractor = new ExtractNumbers();
    expect(extractor.extractNumbers('Temperature: -15')).toEqual(['-15']);
    expect(extractor.extractNumbers('Temperature: -15°C, yesterday: -22°C')).toEqual([
      '-15',
      '-22',
    ]);
  });
  ...
  test('get negative decimal numbers', () => {
    ...
    expect(extractor.extractNumbers('Temperature: -15.7°C, yesterday: -22.33°C')).toEqual([
      '-15.7',
      '-22.33',
    ]);
  });
  ...
  test('get negative numbers with commas', () => {
    const extractor = new ExtractNumbers();
    expect(
      extractor.extractNumbers("Can't think of some -150,000 example, but it should work")
    ).toEqual(['-150,000']);
    expect(extractor.extractNumbers('-170,000, -222,987 and -222,987,899 work too!')).toEqual([
      '-170,000',
      '-222,987',
      '-222,987,899',
    ]);
  ...
  test('get negative decimal numbers with commas', () => {
    const extractor = new ExtractNumbers();
    expect(extractor.extractNumbers('-170,000.77, -222,987.66 and -222,987,899.29')).toEqual([
      '-170,000.77',
      '-222,987.66',
      '-222,987,899.29',
    ]);
  ...
  test('get numbers instead of strings when `string:false` is passed in options', () => {
    ...
    expect(
      extractor.extractNumbers('Your balance: 100,000,000.77, previous month: 90,899,232.89')
    ).toEqual([100000000.77, 90899232.89]);
    expect(extractor.extractNumbers('Temperature: -15, yesterday: -22')).toEqual([-15, -22]);
    expect(extractor.extractNumbers('Temperature: -15.7')).toEqual([-15.7]);
    expect(extractor.extractNumbers('-170,000.77, -222,987.66 and -222,987,899.29')).toEqual([
      -170000.77, -222987.66, -222987899.29,
    ]);
    expect(extractor.extractNumbers('-170,000, -222,987 and -222,987,899')).toEqual([
      -170000, -222987, -222987899,
    ]);
  });
